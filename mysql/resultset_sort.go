...
func (r *resultsetSorter) Less(i, j int) bool {
	...
	for _, k := range r.sk {
		v := cmpValue(v1[k.column], v2[k.column], r.Fields[k.column].Type)
...
func cmpValue(v1 interface{}, v2 interface{}, typ byte) int {
	if v1 == nil && v2 == nil {
		return 0
	} else if v1 == nil {
		return -1
	...
	case string:
		s := v2.(string)
		return bytes.Compare([]byte(v), []byte(s))
	case []byte:
		s := v2.([]byte)
		switch typ {
		case TypeTiny, TypeShort, TypeLong, TypeFloat, TypeDouble, TypeTimestamp, TypeInt24:
			f1, _ := strconv.ParseFloat(string(v), 64)
			f2, _ := strconv.ParseFloat(string(s), 64)
			if f1 < f2 {
				return -1
			} else if f1 > f2 {
				return 1
