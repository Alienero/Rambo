...
func (node *Update) Nodes() []SQLNode {
	return []SQLNode{node.Comments, node.Table,
		node.Exprs, node.Where, node.OrderBy, node.Limit}
}
...
func (node *Set) Nodes() []SQLNode {
	return []SQLNode{node.Comments, node.Exprs}
}
...
func (node *IsExpr) Nodes() []SQLNode {
	return []SQLNode{node.Expr, StrVal(node.Operator)}
}
...
func (node *KeyrangeExpr) Nodes() []SQLNode {
	return []SQLNode{node.Start, node.End}
}
...
func (node *FuncExpr) Nodes() []SQLNode {
	return []SQLNode{StrVal(node.Name), BoolVal(node.Distinct), node.Exprs}
}
...
func (node *CaseExpr) Nodes() []SQLNode {
	...
	for _, e := range node.Whens {
		sns = append(sns, e)
	}
	if node.Else != nil {
		sns = append(sns, node.Else)
	}
	return sns
}
...
func (node *When) Nodes() []SQLNode {
	return []SQLNode{node.Cond, node.Val}
}
...
func (node *Limit) Nodes() []SQLNode {
	...
	sns := make([]SQLNode, 0, 1)
	if node.Offset != nil {
		sns = append(sns, node.Offset)
	}
	return append(sns, node.Rowcount)
...
func (node *UpdateExpr) Nodes() []SQLNode {
	return []SQLNode{node.Name, node.Expr}
}
